...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (member.permissions.has("Administrator")) return client.deleteMsg(message, `${client.e.error} Người này là mod/admin, tôi không thể ban được`, 5000, "reply");
    ...
    if (
      ...
      !member.banable
    ) return client.deleteMsg(message, `${client.e.error} Không thể ban người dùng này được!`, 5000, "reply");

    if (member.roles.highest.position >= message.guild.members.me.roles.highest.position) return client.deleteMsg(message, `${client.e.error} Không thể ban người này vì người này có vai trò cao hơn tôi`, 5000, "reply");
