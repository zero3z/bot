...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (member.permissions.has("Administrator")) return client.deleteMsg(message, `${client.e.error} Người này là mod/admin, tôi không thể mute được`, 5000, "reply");
    ...
    if (
      ...
      member.user.bot ||
      !member.moderatable
    ) return client.deleteMsg(message, `${client.e.error} Tôi không thể mute người dùng này được!`, 5000, "reply");

    if (member.roles.highest.position >= message.guild.members.me.roles.highest.position) return client.deleteMsg(message, `${client.e.error} Tôi không thể mute người này vì người này có vai trò cao hơn tôi`, 5000, "reply");
