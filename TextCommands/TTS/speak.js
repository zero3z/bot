...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!channel) return client.deleteMsg(message, `${client.e.error} Vào kênh thoại để sử dụng lệnh này`, 5000, "reply");

    const string = args.join(" ");
    if (!string) return client.deleteMsg(message, `${client.e.error} Nhập tin nhắn để bot nói!`, 5000, "reply");
    if (string.length > 200) return client.deleteMsg(message, `${client.e.error} Nhập dưới 200 từ!`, 5000, "reply");
